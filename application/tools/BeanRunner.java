...
class BeanRunner {
	...
	static void main (
		...
		new BeanRunner ()
			.primaryProjectName (arguments.get (0))
			.primaryProjectPackageName (arguments.get (1))
			.layerNames (Arrays.asList (arguments.get (2).split (",")))
			.configNames (Arrays.asList (arguments.get (3).split (",")))
			.runnerName (arguments.get (4))
			.methodName (arguments.get (5))
			.runnerArgs (arguments.subList (6, arguments.size ()))
			.run ();
